...
function StatCard({ label, value, displayValue }) {
  ...
  return (
    <div
      className={[
        'bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-gray-100 dark:border-gray-800',
        'transition-all duration-300',
        highlight ? 'ring-2 ring-accent/70 shadow-lg scale-[1.01]' : 'hover:shadow-md',
      ].join(' ')}
    >
      <p className="form-label text-textMuted dark:text-gray-400 text-xs">{label}</p>
...
export default function TrainingPage() {
  ...
  // When status reports training crashed on startup, show log tail in banner
  useEffect(() => {
  ...
  useEffect(() => {
    if (wasRunning && !runningForThisGame && isDone) {
      setBanner({
        type: 'success',
        message: `Training completed for ${gameDisplayNames?.[selectedGame] || selectedGame}.`,
      })
    }
    setWasRunning(runningForThisGame)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [runningForThisGame, isDone])

  const lastUpdatedAgo =
    typeof progress?.updated_at === 'number' ? Math.max(0, Math.round(nowTs - progress.updated_at)) : null
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-8 space-y-6">
        {banner && (
          <div
            className={[
              'rounded-lg px-4 py-3 text-sm shadow-sm flex items-center gap-2',
              banner.type === 'success'
                ? 'border border-emerald-300 dark:border-emerald-900/60 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-900 dark:text-emerald-100'
                : banner.type === 'error'
                  ? 'border border-red-300 dark:border-red-900/60 bg-red-50 dark:bg-red-900/20 text-red-900 dark:text-red-100'
                  : 'border border-sky-300 dark:border-sky-900/60 bg-sky-50 dark:bg-sky-900/20 text-sky-900 dark:text-sky-100',
            ].join(' ')}
          >
          ...
          <>
            ...
            <div className="glass-card p-8 space-y-8">
                  ...
                  {progress?.updated_at && (
                    <p className="text-xs text-textLight dark:text-gray-500 mt-1.5 flex items-center gap-1">
                      ...
                      <span>
                        ...
                        {lastUpdatedAgo != null && (
                          <span className="ml-1 text-[0.7rem] text-textMuted dark:text-gray-500">
                ...
                {percent != null ? (
                  <div className="space-y-3">
                    <div className="w-full h-4 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden border border-gray-200 dark:border-gray-700 p-0.5">
                      <div
                        className={[
                          'h-full bg-gradient-to-r from-accent to-blue-400 rounded-full transition-all duration-1000',
                          runningForThisGame ? 'animate-pulse' : '',
                        ].join(' ')}
                        style={{ width: `${percent}%` }}
                      />
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard
                  label="Algorithm"
                  value={progress?.algo}
                  displayValue={(progress?.algo || 'ppo').toUpperCase()}
                />
                <StatCard
                  label="Episodes"
                ...
                />
                <StatCard
                  label="Mean reward (recent eps)"
                  value={progress?.reward}
                  displayValue={
                    ...
                    <tbody>
                      {history
                        ...
                        .map((run) => {
                          ...
                          return (
                            <tr
                              key={run.run_id || `${run.game_key}-${run.updated_at || Math.random()}`}
            ...
            {history.length === 0 && (
              <p className="text-xs text-textMuted dark:text-gray-500">
                No runs yet for this game. Start training to see history here.
              </p>
            )}
          </>
        )}
